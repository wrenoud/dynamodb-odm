...
var Model = function Model(name, hashKey, schema, connection){
     ...
     */
    instance.query = function(hash, options, callback){
        var schema = this.schema;
        var prepared_hash = {}
        for(var path in hash){
            ...
            if(typeof hash[path] === 'object'){
                for(var cond in hash[path]){
                    // if there is a condition we have to dive into the object
                    prepared_hash[path] = {};
                    prepared_hash[path][cond] = schema.paths[path].cast(hash[path][cond]);
                }
            }else{
                prepared_hash[path] = schema.paths[path].cast(hash[path]);
            }
        }
        this.connection.query(this.modelName, hash, options, function(err,res,con){
            if(err){
                callback(err);
            }else{
                var items = res.items;
    ...
    instance.create = function(){
        ...
        if(paths[this.hash]){
            ...
            this.connection.createTable(this.modelName, hash, lsi);
        }else{
