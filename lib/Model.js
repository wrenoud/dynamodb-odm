        ...
        for(var path in hash){
            ...
            if(typeof hash[path] === 'object'){
                for(var cond in hash[path]){
                    ...
                    prepared_hash[path] = {};
                    prepared_hash[path][cond] = this.schema.paths[path].cast(hash[path][cond])
                }
            }else{
                prepared_hash[path] = this.schema.paths[path].cast(hash[path])
            }
        }
        this.connection.query(this.modelName, hash, options, callback);
    };
